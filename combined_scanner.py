...
Schedule  : 09:00-22:00 UTC Mon-Fri (4:30 AM - 6:00 PM ET, covers pre + post market)
"""
...
def get_yf_news(stock_obj, n: int = 3) -> str:
    try:
        return "\n".join(f"- {item['title']}" for item in stock_obj.news[:n])
    except Exception:
        return "- No recent news available."

def get_newsapi_headlines(n: int = 5) -> list:
    if not NEWS_API_KEY:
...
def get_cien_technical_strength() -> dict:
    cien = yf.Ticker("CIEN")
    df   = cien.history(period="1d", interval="5m", prepost=True)  # prepost=True for pre/post-market
    ...
    if df.empty or len(df) < 15:
        print("[CIEN] Not enough 5-min bars.")
        return result
    ...
    if rising and vol_surge:
        signal, details = "STRONG UP", f"RSI: {rsi}. Heavy buy volume."
        sl, tp = round(current_price - 1.5 * atr, 2), round(current_price + 3.0 * atr, 2)
    elif not rising and vol_surge:
        signal, details = "STRONG DOWN", f"RSI: {rsi}. Heavy sell volume."
        sl, tp = round(current_price + 1.5 * atr, 2), round(current_price - 3.0 * atr, 2)

    result.update({"signal": signal, "details": details, "price": current_price,
                   "pct_change": pct_change, "stop_loss": sl, "take_profit": tp, "atr": atr})
    return result

def get_gemini_sentiment(price: float, signal: str, details: str, headlines: list) -> tuple:
    prompt = f"""
...
def analyze_vsa(ticker: str):
    stock = yf.Ticker(ticker)
    df    = stock.history(period=VSA_CONFIG["data_period"],
                          interval=VSA_CONFIG["data_interval"], auto_adjust=True)
    ...
    df.ta.rsi(length=VSA_CONFIG["rsi_period"], append=True)
    ...
    if (price < sma_20 and vol > vol_mult * avg_vol and price > open_p and rsi < VSA_CONFIG["rsi_accumulation_max"]):
        return (f"🟢 <b>ACCUMULATION: {ticker}</b>\n"
                f"Price: ${price:.2f} | RSI: {rsi:.1f} | SMA20: ${sma_20:.2f}\n"
                f"Volume: {vol_ratio:.1f}x average\n\n"
                f"<b>Headlines:</b>\n{get_yf_news(stock)}")

    elif (price > sma_20 and vol > vol_mult * avg_vol and price < open_p and rsi > VSA_CONFIG["rsi_distribution_min"]):
        return (f"🔴 <b>DISTRIBUTION: {ticker}</b>\n"
                f"Price: ${price:.2f} | RSI: {rsi:.1f} | SMA20: ${sma_20:.2f}\n"
                f"Volume: {vol_ratio:.1f}x average\n\n"
                f"<b>Headlines:</b>\n{get_yf_news(stock)}")
    return None

def run_vsa_scanner(tickers: list):
    print(f"[VSA Scanner] Scanning {len(tickers)} ticker(s)...")
    signals = [s for t in tickers if (s := analyze_vsa(t))]
    if signals:
        send_telegram_message("\n\n---\n\n".join(signals), parse_mode="HTML")
        print(f"[VSA] {len(signals)} signal(s) sent.")
    else:
        send_telegram_message("<b>VSA Scanner</b>\nNo institutional volume anomalies detected.", parse_mode="HTML")
        print("[VSA] No signals.")
